...
<body>
    <div class="container">
        <%
            ...
            Teacher updateTeacher = (Teacher) request.getAttribute("updateTeacher");
            Faculity faculityInfo = (Faculity) request.getAttribute("faculityInfo");
        %>
        <h1>Edit Teacher</h1>
        <a href="ViewListTeacher?faculityId=<%=faculityInfo.getId() %>" class="btn btn-back mb-3"><i class="fa-solid fa-backward"></i> Back</a>
        <div class="form-container">
            <form action="CRUDTeacher?mode=<%= mode %>" method="post">
            	<input type="hidden" name="faculityId" value="<%= faculityInfo.getId() %>">
                <% if (mode == 3) { %>
                    <input type="hidden" name="id" value="<%= updateTeacher.getId() %>">
                <% } %>
                <div class="form-group">
                    <label for="name">Name:</label>
                    <input type="text" class="form-control" id="name" name="name" value="<%= updateTeacher != null ? updateTeacher.getName() : "" %>">
                </div>
                <div class="form-group">
                    <label for="email">Email:</label>
                    <input type="text" class="form-control" id="email" name="email" value="<%= updateTeacher != null ? updateTeacher.getEmail() : "" %>">
                </div>
                <div class="form-group">
